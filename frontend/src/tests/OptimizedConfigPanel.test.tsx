...
describe("OptimizedConfigPanel", () => {
  ...
  it("renders adaptation notes for the Terraform path", () => {
    render(
      <OptimizedConfigPanel
        config={{
          ...optimizedConfig,
          ...
          changes: [],
          notes: [
            "Autoscaling has no Terraform (aws_instance) semantics; the artifact provisions the recommended fixed replica count.",
          ],
        }}
        loading={false}
        error={null}
    ...
    expect(screen.getByTestId("config-notes")).toHaveTextContent(/no Terraform.*semantics/i);
    expect(screen.queryByTestId("config-changes")).toBeNull();
  });

  it("renders adaptation notes for the Compose path", () => {
    render(
      <OptimizedConfigPanel
        config={{
          ...optimizedConfig,
          optimized_yaml: "# Generated by EcoOps AI\nservices:\n  web:\n    image: nginx",
          changes: [],
          notes: ["The 10.5 GB storage setting is not expressible in a Compose service definition."],
        }}
        loading={false}
        error={null}
        onGenerate={vi.fn()}
    ...
    expect(screen.getByTestId("config-notes")).toHaveTextContent(/not expressible/i);
  });
});

...
def test_optimized_config_rejected_for_terraform_with_clear_message() -> None:
    analyze_response = client.post(
        "/api/v1/analyze",
        files=_tf_file(),
        data={"workload": json.dumps(TERRAFORM_WORKLOAD)},
    ...
    assert "Kubernetes" in config_response.json()["detail"]
    assert "Phase 16" in config_response.json()["detail"]

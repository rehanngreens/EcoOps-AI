"""Phase 13 live smoke test: Docker Compose end-to-end through the real API."""

import json
import urllib.error
import urllib.request

BASE = "http://127.0.0.1:8142/api/v1"
COMPOSE = "infrastructure/docker/compose-heavy-overprovisioned.yaml"
K8S = "infrastructure/kubernetes/deployment-heavy-overprovisioned.yaml"
WORKLOAD = {
...
# 1. validate
body, ctype = multipart({"file": ("compose.yaml", compose_text, "application/x-yaml")})
...
body, ctype = multipart(
    {"file": ("compose.yaml", compose_text, "application/x-yaml")},
...
print(f"  config:       {config['source_type']} app={config['application']} "
      ...
      f"mem={config['memory_request_gb']} image={config['container_image']}")
print(f"  warnings:     {config.get('parser_warnings')}")
...
assert status == 201 and config["source_type"] == "docker_compose"
...
# 4. optimized-config must refuse Compose with the generalized message
status, payload = request("GET", f"/analysis/{analysis_id}/optimized-config")
print(f"optimized-conf: HTTP {status} detail={payload['detail'][:80]}…")
assert status == 400 and "Docker Compose" in payload["detail"]

# 5. regressions: Kubernetes and Terraform flows untouched
k8s_text = open(K8S, encoding="utf-8").read()
body, ctype = multipart(
...
request("POST", f"/analysis/{kid}/optimize")
...
tf_text = open("infrastructure/terraform/main-heavy-overprovisioned.tf", encoding="utf-8").read()
body, ctype = multipart(
    {"file": ("main.tf", tf_text, "text/x-terraform")},
    {"workload": json.dumps(WORKLOAD)},
)
status, payload = request("POST", "/analyze", body, ctype)
assert status == 201 and payload["configuration"]["source_type"] == "terraform"
print("tf regression:  analyze HTTP 201 source_type=terraform")

print("\nSMOKE TEST PASSED — all three IaC formats work; Compose is a first-class Mode A input.")
